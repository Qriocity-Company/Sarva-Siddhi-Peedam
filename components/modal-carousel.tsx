...
const ModalCarousel = ({images,setOpen,videos}:ModalCarouselProps) => {
  ...
  return (
    <div className='fixed top-0 left-0 w-full h-[100vh] bg-[rgba(0,0,0,0.8)] flex justify-center items-center px-20'>
        <X className='z-20 h-8 w-8 md:h-10 md:w-10 absolute right-3 top-4 cursor-pointer text-white' onClick={handleClose}/>
        <Carousel opts={{
                loop:true
            }} className='w-[120%] md:w-[100%] '>
            <CarouselContent className=''>
                {
                    images.map((img,index)=>{
                        return(
                            <CarouselItem key={index} className='basis1/1 mx-5 flex items-center'>
                                <Image src={img} alt='img' className=' md:h-[650px] w-auto m-auto'></Image>
                            </CarouselItem>
                        )
                    })
                }
                    ...
                    videos.map((url,index)=>{
                        return(
                            <CarouselItem key={index} className='basis1/1 mx-5 flex items-center'>
                                <iframe 
                                    src={url}
                                    className='md:w-[600px] md:h-[650px] w-auto m-auto'
